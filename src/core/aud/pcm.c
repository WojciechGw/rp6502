...
static void
    HOST_ISR
    HOST_TIME_CRITICAL(pcm_irq_handler)(void)
{
    aud_clear_irq();

    // Output previous sample at start to minimize jitter
    aud_out(pcm_sample_l, pcm_sample_r);
    ...
    if (((write_ptr - pcm_read_ptr) & mask) >= fsz)
    {
        ...
        if (fmt & 2)
        {
            ...
            if (fmt & 4)
            {
                next_l = (int16_t)((int16_t)raw_l - 128) << 8;
                next_r = (int16_t)((int16_t)raw_r - 128) << 8;
            }
            else
            {
                next_l = (int8_t)raw_l << 8;
                next_r = (int8_t)raw_r << 8;
            }
        }
        else
        {
    ...
    int32_t mix_l = next_l + bel_mix;
    int32_t mix_r = next_r + bel_mix;
    if (mix_l < AUD_SAMPLE_MIN)
        mix_l = AUD_SAMPLE_MIN;
    if (mix_l > AUD_SAMPLE_MAX)
        mix_l = AUD_SAMPLE_MAX;
    if (mix_r < AUD_SAMPLE_MIN)
        mix_r = AUD_SAMPLE_MIN;
    if (mix_r > AUD_SAMPLE_MAX)
        mix_r = AUD_SAMPLE_MAX;
    pcm_sample_l = (int16_t)mix_l;
    pcm_sample_r = (int16_t)mix_r;
